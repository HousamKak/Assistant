...
namespace VoiceAssistant.Core.Services
{
    ...
    public class AssistantService : IAssistantService
    {
        ...
        private void OnWakeWordDetected(object sender, string keyword)
        {
            ...
            // Speak the wake word response phrase if text-to-speech is enabled
            if (_configuration.TextToSpeech.Enabled && !string.IsNullOrEmpty(_configuration.WakeWord.ResponsePhrase))
            {
                try
                {
                    Task.Run(async () =>
                    {
                        try
                        {
                            _logger.LogInformation("Speaking wake word response: \"{ResponsePhrase}\"",
                                _configuration.WakeWord.ResponsePhrase);
                            await _textToSpeechService.SpeakAsync(_configuration.WakeWord.ResponsePhrase, _cts.Token);
                        }
                        catch (Exception ex)
                        {
                            _logger.LogError(ex, "Error speaking wake word response");
                        }
                    });
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Failed to start speech task for wake word response");
                }
            }
